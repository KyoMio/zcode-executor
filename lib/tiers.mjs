// 本文件负责：模型等级（fast / strong）的纯函数分配、思考等级校验与 3.12 起 create 必带的
// reasoningLevel 回落（SPEC「工作流层」、PRD 第 5 节、D6）。
// 不负责：拿模型清单（3.12 起由 lib/models.mjs 从 registry 本地换算）、选 provider（lib/providers.mjs
// 的 pickProvider）、配置文件读取。
// 被依赖方：lib/models.mjs、lib/run.mjs、lib/cli/new.mjs。只依赖 lib/errors.mjs。
//
// 输入形状：settings.model.available 的条目（lib/models.mjs 从 registry 换算成同一形状）
// { ref:{providerId, modelId}, label?, reasoning?:{levels:[{value,label}], defaultLevel?}, disabledReason? }。
...
/**
 * 纯函数：runner 建会话时用的 reasoningLevel。3.12.2 起 GLM 5.3 系列 create 缺 options.reasoningLevel
 * 直接被拒「Reasoning level is required」（verified.md「3.12.2 直连探针实测」表第 3 行，2026-09-18），而登记簿的 thoughtLevel
 * 可能是 null（new 在模型没有 high 档时存 null），所以要回落：登记簿值 → 模型 reasoning.defaultLevel → 'high'。
 * model 是 registry 的 provider.models 元素（{modelId, reasoning?:{defaultLevel?}}），传 undefined 也不抛，
 * 让 create 自己去拒——这里只管补档位。
 */
export function reasoningLevelFor(thoughtLevel, model) {
  // 权宜：最后一档写死 high；config.json 里 reasoning 缺 variants 的模型（本机三个都有 low/high/max）
  // 会把 high 送给 strict schema 的 create——真机见到这种模型再改成 levels[0]
  return thoughtLevel ?? model?.reasoning?.defaultLevel ?? 'high';
}
