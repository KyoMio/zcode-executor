...
export async function run(argv) {
  ...
  // --steer 前置检查（2026-09-21 对照 ZCode 源码：3.12.2 起回合中 session/send 直接被拒 -32010，
  // 插话路径废了）：runner 已确认插话不可用（steerUnavailable 非空）就提前拒绝，不入队、不起 runner
  if (steer) {
    const state = readState(config.home, id);
    if (state?.phase === 'running' && state.steerUnavailable) {
      throw new ExecutorError(`本会话的插话路径不可用：${state.steerUnavailable}。等回合结束后再投，或先 cancel`, 2);
    }
  }
