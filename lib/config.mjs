...
function readSecretConfig(configPath) {
  let fd;
  try {
    fd = openSync(configPath, constants.O_RDONLY | constants.O_NOFOLLOW);
    const stat = fstatSync(fd);
    if (!stat.isFile()) {
      throw new ExecutorError(`含 Jev API key 的配置必须是普通文件：${configPath}`, 1);
    }
    if (typeof process.getuid === 'function' && stat.uid !== process.getuid()) {
      throw new ExecutorError(`含 Jev API key 的配置必须属于当前用户：${configPath}`, 1);
    }
    if ((stat.mode & 0o077) !== 0) {
      throw new ExecutorError(`含 Jev API key 的配置权限过宽：${configPath}。运行 chmod 600 ${configPath}`, 1);
    }
    return readFileSync(fd, 'utf8');
  } catch (err) {
    if (err instanceof ExecutorError) throw err;
    if (err?.code === 'ELOOP') {
      throw new ExecutorError(`含 Jev API key 的配置不能是符号链接，必须是普通文件：${configPath}`, 1);
    }
    throw new ExecutorError(`无法安全读取含 Jev API key 的配置 ${configPath}：${err.message}`, 1);
  } finally {
...
function jevKeyOf(given, configPath) {
  const jev = given.review?.jev;
  if (jev === undefined) return undefined;
  if (jev === null || typeof jev !== 'object' || Array.isArray(jev)) {
    throw new ExecutorError(`配置 review.jev 必须是对象（{apiKey}）：${configPath}`, 1);
  }
  if (jev.apiKey === undefined) return undefined;
  if (typeof jev.apiKey !== 'string') {
    throw new ExecutorError(`配置 review.jev.apiKey 必须是字符串：${configPath}`, 1);
  }
  return jev.apiKey.trim() === '' ? undefined : jev.apiKey;
}
...
export function loadConfig(home = resolveHome()) {
  ...
  const config = {
    ...
    // thought 进 selection.options.reasoningLevel；timeoutMs 是一次 generateText 的上限；fastMaxTokens 是快筛
  ...
  let given = parseConfig(raw, configPath);
  const jevKey = jevKeyOf(given, configPath);
  if (jevKey !== undefined) {
    // D15：含长期密钥时必须从通过同一 fd 校验的文件重读，避免 lstat/read 的竞态。
    raw = readSecretConfig(configPath);
    given = parseConfig(raw, configPath);
  }
  ...
  if (given.review !== undefined) {
    ...
    const validatedJevKey = jevKeyOf(given, configPath);
    if (validatedJevKey !== undefined) config.review.jev.apiKey = validatedJevKey;
    if (review.jev !== undefined) {
      for (const key of Object.keys(review.jev)) {
        if (key !== 'apiKey') {
          config.warnings.push(`配置 review.jev 里有不认识的键 ${key}（已忽略），检查是否拼错：${configPath}`);
        }
      }
    }
    for (const key of Object.keys(review)) {
      if (!['enabled', 'model', 'thought', 'fastMaxTokens', 'slowMaxTokens', 'timeoutMs', 'jev'].includes(key)) {
