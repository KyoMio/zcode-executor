...
// 环境变量）、按 id 配对请求、反向请求去重与自动应答（含模型请求前的 provider 运行时头）、通知转发、
// 子进程收场；另有 zcode 二进制的定位与版本探查（findZcode / zcodeVersion）、内置 provider 配置的
// 定位（builtinProviderConfigPath，也是「App ≥ 3.12」的判据）和 doctor、models 用的零 token 握手
// 探针（probeHandshake：create + close）。
