// 本文件负责：zcode app-server 的 JSON-RPC 客户端——拉起子进程（3.12 起要带两个 provider 文件的
// 环境变量）、按 id 配对请求、v4/command 命令信封的发送与 ACK 判定（command）、反向请求去重与
// 自动应答（含模型请求前的 provider 运行时头）、通知转发、子进程收场；另有 zcode 二进制的定位与
// 版本探查（findZcode / zcodeVersion）、内置 provider 配置的定位（builtinProviderConfigPath，
// 也是「App ≥ 3.12」的判据）和 doctor、models 用的零 token 握手探针（probeHandshake：create + close）。
// 不负责：会话生命周期（session/create 等的调用时机），那是 lib/session.mjs 和上层的事；个人 provider
// 文件的内容与落盘（lib/providers.mjs）；也不负责日志落盘。被工作流层调用，只依赖 lib/errors.mjs 和
// lib/scrub.mjs（T0.3b 复核 B）。
// 协议事实见 docs/reference/zcode-app-server-protocol.md、docs/verified.md（2026-09-07）与
// docs/verified.md（3.12.2 探针，2026-09-18）。
//
...
// verified.md「3.12.2 直连探针实测」表第 5 行（2026-09-18）：zcode.cjs 的宿主模式会在模型请求前向客户端要运行时头，
// 等 {headersApplied: true, requestAuth: {apiKey?, headers?}}，180 秒不答模型请求就失败——检查点 5 真机证实
// api-key 型 provider 不发它，留着是为账号型 provider 兜底。key 由 spawn 时给的 providerAuth 按 params.providerId
// 提供；没有就明说 headersApplied:false，让回合立刻失败而不是等满 180 秒。
const RUNTIME_HEADERS_METHOD = 'interaction/requestProviderRuntimeHeaders';

// 3.12 起 app-server 从这两个环境变量指的文件读 provider 表（docs/reference/zcode-app-server-protocol.md「3.12.2 变化」）：
// 两个都给了，CLI 原样采用、不去 CDN 刷新、也不碰用户的 ~/.zcode/v2/provider_config.json
const BUILTIN_FILE_ENV = 'ZCODE_BUILTIN_PROVIDER_CONFIG_FILE';
const PERSONAL_FILE_ENV = 'ZCODE_PERSONAL_PROVIDER_CONFIG_FILE';

// ZCode App 把 zcode.cjs 装在哪：macOS 两处、Linux 两处，抄自 zcode-acp resolve.js 的
// bundledZcodeCandidates()（decisions.md D2）。App 从不把 CLI 放进 PATH，所以只认这几个固定位置，
// 装在别处用 ZCODE_BIN 指。Windows 的位置（%LOCALAPPDATA%\Programs\ZCode\…）还没在真机上验过，
// 先不列——那边一样用 ZCODE_BIN。
...
export function findZcode({ zcodeBin = process.env.ZCODE_BIN } = {}) {
  ...
  // Linux 的安装位置不固定（zcode-acp README：解压出来的应用目录里的 resources/glm/zcode.cjs），
  // 所以找不到时把定位命令一并给出来，省得人去翻文档。
  throw new ExecutorError(
    '找不到 zcode.cjs：这几个位置都没有 ZCode App。设环境变量 ZCODE_BIN 指向 zcode.cjs 绝对路径；'
      + "不知道装在哪就找一下：find / -name zcode.cjs -path '*resources/glm*' 2>/dev/null",
  );
}

/**
 * 从 zcode.cjs 路径推出 App 自带的内置 provider 配置：两个平台的 App 目录布局都是
 * Resources/glm/zcode.cjs 与 Resources/config/provider/zcode-builtin.json 并列（docs/reference/zcode-app-server-protocol.md「3.12.2 变化」）。
 * 文件不存在抛 ExecutorError——3.12 之前的 App 没有这个文件，所以它也是版本门槛
 * （`--version` 在 3.11 和 3.12 都打 0.16.5，区分不了）。
 */
export function builtinProviderConfigPath(zcodePath, { builtinFile = process.env[BUILTIN_FILE_ENV] } = {}) {
  // 环境变量指了就只认它（App 装在别处、或想试 CDN 刷新过的活动副本）；指错了直说，不悄悄回落到默认位置
  if (builtinFile) {
    if (!existsSync(builtinFile)) {
      throw new ExecutorError(
        `环境变量 ${BUILTIN_FILE_ENV} 指向的文件不存在：${builtinFile}。改成 zcode-builtin.json 的绝对路径，或删掉这个环境变量改用默认位置`,
        { zcode: zcodePath, file: builtinFile },
      );
    }
    return builtinFile;
  }
  const file = path.resolve(path.dirname(zcodePath), '../config/provider/zcode-builtin.json');
  if (!existsSync(file)) {
    throw new ExecutorError(
      `找不到 ZCode 内置 provider 配置：${file}。ZCode App 低于 3.12（本插件只支持 3.12 起）或目录布局不同；`
        + `升级 ZCode App，或设环境变量 ${BUILTIN_FILE_ENV} 指向 zcode-builtin.json 的绝对路径`,
      { zcode: zcodePath, file },
    );
  }
  return file;
}
...
/**
 * doctor / models 的零 token 握手探针：spawn → session/create → session/close → 收场，
 * 返回 create 的 settings（model.available 里应有个人文件那个 provider 的模型；current / thoughtLevel
 * 回显所选）。不 send、不 generateText。
 * persistence 用 'deferred'（真机 3.12.2 探针 2026-09-18：deferred 建的会话 create 成功、close 回
 * {closed:true}，前后 session/list 都不列它——doctor 每跑一次不在 App 的任务列表里留一条）。
 * cwd 只当子进程与 workspace 路径，缺省临时目录即可；secrets 与 env 原样给 spawn。
 * model 形状 {providerId, modelId, options:{reasoningLevel}}（buildModelSelection），
 * 不给就让 app-server 用表里第一个模型。
 */
export async function probeHandshake({
  ...
  const client = await AppServerClient.spawn({ cwd, zcodePath, env, secrets, personalProviderFile, providerAuth });
  try {
    const params = { workspace, mode: 'build', persistence: 'deferred', titleGenerationEnabled: false };
    if (thoughtLevel !== undefined) params.thoughtLevel = thoughtLevel;
    if (model !== undefined) params.model = model;
    const created = await client.request('session/create', params, { timeoutMs });
    await client.request('session/close', { sessionId: created.session.sessionId }, { timeoutMs });
 ...
 *   interaction/requestProviderRuntimeHeaders 没被接住时，按 providerAuth 给的 key 回
 *   {headersApplied:true, requestAuth:{apiKey}}，没有 key 回 {headersApplied:false, errorMessage}。
 */
export class AppServerClient {
   ...
   * @param {string} [opts.personalProviderFile] 个人 provider 文件绝对路径（lib/providers.mjs 的
   *   writePersonalProviderFile 写的）。进 ZCODE_PERSONAL_PROVIDER_CONFIG_FILE，优先级：这里 > opts.env >
   *   process.env > 抛 ExecutorError（没有个人文件 create 一定失败，早报）
   * @param {(providerId: string) => (string|undefined)} [opts.providerAuth] 答 requestProviderRuntimeHeaders
   *   时按 providerId 取 apiKey；没给或返回空 → headersApplied:false
   * @param {string[]} [opts.secrets] 要从本进程 stderr 转发里抹掉的密钥值（如 registry 里的 apiKey）；
   *   providerAuth 答出去的 key 会自动加进来
  ...
  static async spawn(opts = {}) {
    ...
    delete env.ZCODE_MODEL;
    // docs/reference/zcode-app-server-protocol.md「3.12.2 变化」：两个 provider 文件的环境变量。显式参数 > opts.env > process.env；
    // 内置文件还能从 zcode.cjs 位置推出来，个人文件推不出来——没有它 create 必失败，在这里就报
    if (personalProviderFile) env[PERSONAL_FILE_ENV] = personalProviderFile;
    if (!env[PERSONAL_FILE_ENV]) {
      throw new ExecutorError(
        `拉起 zcode app-server 缺个人 provider 文件：spawn 没给 personalProviderFile，环境里也没有 ${PERSONAL_FILE_ENV}。`
          + '先用 lib/providers.mjs 的 writePersonalProviderFile 写一份再拉起',
        { zcode },
      );
    }
    env[BUILTIN_FILE_ENV] = builtinProviderConfigPath(zcode, { builtinFile: env[BUILTIN_FILE_ENV] });
  ...
  /**
   * 发一条 v4 命令并等 ACK（2026-09-21 对照 ZCode 源码 + 本机探针 App 3.12.2）。v4 与旧
   * session/* 走同一条 NDJSON 流，params 就是命令信封（commandId/clientId/sessionId/type/
   * issuedAt/payload），不需要先订阅 v4 主题、不需要握手——网关只查会话是否存在，旧
   * session/create 建的会话同样可用。
   * ACK 的 status 为 accepted | noop | duplicate 视为成功；其余（rejected | stale | failed）
   * 抛 ExecutorError，details 带 { type, status, reasonCode, message, commandId }。
   * @returns {Promise<object>} ACK（{commandId, status, revisionAtDecision, reasonCode?, message?, result?}）
   */
  async command(sessionId, type, payload, { timeoutMs } = {}) {
    const envelope = {
    ...
    const ack = await this.request('v4/command', envelope, { timeoutMs });
    const status = ack?.status;
    if (status !== 'accepted' && status !== 'noop' && status !== 'duplicate') {
      const { reasonCode, message } = ack ?? {};
      throw new ExecutorError(
        `v4 命令 ${type} 未被接受：${status ?? '无 status'}（${reasonCode ?? '无原因'}）${message ? '：' + message : ''}`,
        { type, status, reasonCode, message, commandId: envelope.commandId },
      );
    }
    return ack;
  }
  ...
  async #dispatchServerRequest(msg, entry) {
    ...
    let thrown = null;
    try {
      // 处理器没接住（返回 undefined）就用内置应答；两边抛错都按 -32603 回，不让 zcode 干等
      answer = await this.#opts.onServerRequest?.({ id: msg.id, method: msg.method, params });
      if (answer === undefined) answer = this.#builtinAnswer(msg.method, params);
    } catch (err) {
      thrown = err;
    }
    if (thrown !== null) {
  ...
  // 内置应答：处理器没接住的两种反向请求。其它方法返回 undefined（调用方回 -32601）
  #builtinAnswer(method, params) {
    if (method === RUNTIME_PREFERENCES_METHOD) return DEFAULT_RUNTIME_PREFERENCES;
    if (method === RUNTIME_HEADERS_METHOD) {
      const apiKey = this.#opts.providerAuth?.(params.providerId);
      if (typeof apiKey !== 'string' || !apiKey) { // 回调是同步的：返回 Promise 会被序列化成 {}，按没 key 处理
        this.#note(`appserver: 没有 provider ${params.providerId ?? '(缺 providerId)'} 的 API key，这次模型请求会失败`);
        return {
          headersApplied: false,
          errorMessage: `zcode-executor 没有 provider ${params.providerId ?? ''} 的 API key，模型请求发不出去`,
        };
      }
      // key 一旦答出去，stderr 转发就可能带出它（zcode 的日志行）：加进抹除名单
      if (!this.#opts.secrets.includes(apiKey)) this.#opts.secrets = [...this.#opts.secrets, apiKey];
