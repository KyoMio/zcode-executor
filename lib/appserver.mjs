...
// docs/PLAN-3.12.md（3.12.2 探针，2026-09-18）。
//
...
// PLAN-3.12.md 一节层 5（2026-09-18）：宿主模式下 app-server 每次模型请求前都发这个反向请求，等客户端
...
// 3.12 起 app-server 从这两个环境变量指的文件读 provider 表（PLAN-3.12.md 二节第 1 条）：
 ...
 * Resources/glm/zcode.cjs 与 Resources/config/provider/zcode-builtin.json 并列（PLAN-3.12.md 二节第 1 条）。
...
export class AppServerClient {
  ...
  static async spawn(opts = {}) {
    ...
    delete env.ZCODE_MODEL;
    // PLAN-3.12.md 二节第 1 条：两个 provider 文件的环境变量。显式参数 > opts.env > process.env；
