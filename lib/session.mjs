...
export function settleTurn(events) {
  ...
  if (completed) {
    ...
    if (resultType === 'cancelled') {
      return { outcome: 'cancelled', reason: '回合被叫停（resultType=cancelled）', lastText, usage };
    }
    return { outcome: 'failed', reason: `turn.completed resultType=${resultType}`, lastText, usage };
  }
  if (failed) {
...
export class Session {
  ...
  /**
   * 投递一条消息并等本回合结束。
   * @returns {Promise<{outcome:'done'|'failed'|'cancelled'|'timeout'|'exited', reason:string|null, startedAt, endedAt, lastText:string, usage:object|null}>}
