// 本文件负责：Jev 可选前筛 → 原 ZCode 快筛 → 必要时慢判，createReview 返回
// review(action, ctx)。Jev 仅 pass 提前放行，其余完整保留原两段审批。
...
export function createReview(complete, options = {}) {
  ...
  return async function review(action, ctx) {
    ...
    if (fastScreen) {
      try {
        const screened = await fastScreen(action, ctx);
        if (screened?.decision === 'pass' && screened?.outcome === 'pass') {
          preScreen = { ...screened.metadata, reviewer: 'jev', outcome: 'pass', reasonCode: 'pass' };
          return { ...screened.metadata, decision: 'allow', stage: 'review-fast', reason: 'Jev 快筛通过', reviewer: 'jev', preScreen };
        }
        const outcome = ['flag', 'skip'].includes(screened?.outcome) ? screened.outcome : 'error';
        preScreen = { ...screened?.metadata, reviewer: 'jev', outcome };
        // fastReview 兼容旧事件：只表示 Jev flag/error，不重用为 ZCode 的快筛结果。
        if (outcome !== 'skip') fastReview = { ...preScreen };
      } catch {
        // adapter 的预期失败会返回 typed error；意外异常也回原链路，不记异常正文。
        preScreen = { reviewer: 'jev', outcome: 'error', errorCode: 'adapter_error' };
        fastReview = { ...preScreen };
      }
    }
    {
      let fastText;
      try {
        ...
        return withAudit({ decision: 'ask', stage: 'review-failed', reason: errorText(e) });
      }
      ...
      if (parseFast(fastText) === 'pass') {
        return withAudit({
          decision: 'allow',
          stage: 'review-fast',
          reason: `快筛通过（模型回 ${fastText.trim().slice(0, 20)}）`,
        });
      }
      // ZCode 阶段一只负责挑出「可能超范围」的，纠偏全靠阶段二，这里不做任何判断。
    }
    ...
    if (parsed === undefined) {
      ...
      return withAudit({ decision: 'ask', stage: 'review-failed', reason: `结论行解析失败，模型末尾: ${tail}` });
    }
    ...
    if (parsed.decision === 'deny') {
      ...
      return withAudit({ decision: 'ask', stage: 'review-slow', reason: parsed.reason, ruleId: parsed.ruleId });
    }
    if (parsed.decision === 'ask') {
      // parseSlow 把「说不出规则的拒绝」降级成了 ask，理由带前缀，原样透传
      return withAudit({ decision: 'ask', stage: 'review-slow', reason: parsed.reason });
    }
    return withAudit({
      decision: 'allow',
      stage: 'review-slow',
