...
const REVIEW_METADATA_KEYS = [
  'reviewer', 'reviewModel', 'reviewSchema', 'probabilities', 'usage', 'requestId',
  'attempts', 'status', 'durationMs',
];
const FAST_REVIEW_KEYS = [...REVIEW_METADATA_KEYS, 'outcome', 'errorCode'];
const PRE_SCREEN_KEYS = [...FAST_REVIEW_KEYS, 'reasonCode'];
const REASON_CODES = new Set(['pass', 'risk_flag', 'evidence_omitted', 'input_truncated', 'missing_intent', 'unsafe_input']);
const ERROR_CODES = new Set(['adapter_error', 'deadline_exceeded', 'invalid_response', 'network_error']);
const PROBABILITY_KEYS = ['scope_conflict', 'outside_worktree_write', 'credential_or_exfiltration', 'destructive_or_external', 'unrelated_or_gratuitous'];

function numberMap(value, keys, valid) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) return undefined;
  const picked = {};
  for (const key of keys) if (valid(value[key])) picked[key] = value[key];
  return Object.keys(picked).length ? picked : undefined;
}

// 外部诊断值不能因字段名在 allowlist 就被信任；旧事件仍可读，新事件只写有界类型。
function pickMetadata(value, keys, secret) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) return undefined;
  const picked = {};
  const count = (n) => Number.isSafeInteger(n) && n >= 0;
  for (const key of keys) {
    const item = value[key];
    if (typeof item === 'string' && secret && item.includes(secret)) continue;
    let valid = false;
    switch (key) {
      case 'reviewer': valid = ['jev', 'zcode'].includes(item); break;
      case 'reviewModel': valid = item === 'jev-1.13.0'; break;
      case 'reviewSchema': valid = ['approval-v1', 'approval-v2'].includes(item); break;
      case 'outcome': valid = ['pass', 'flag', 'error', 'skip'].includes(item); break;
      case 'reasonCode': valid = REASON_CODES.has(item); break;
      case 'errorCode': valid = ERROR_CODES.has(item) || (typeof item === 'string' && /^http_[45]\d{2}$/.test(item)); break;
      case 'requestId': valid = typeof item === 'string' && /^[A-Za-z0-9][A-Za-z0-9_.:-]{0,127}$/.test(item); break;
      case 'attempts': valid = count(item); break;
      case 'status': valid = Number.isInteger(item) && item >= 100 && item <= 599; break;
      case 'durationMs': valid = Number.isFinite(item) && item >= 0; break;
      case 'probabilities': {
        const probabilities = numberMap(item, PROBABILITY_KEYS, (n) => Number.isFinite(n) && n >= 0 && n <= 1);
        if (probabilities) picked[key] = probabilities;
        break;
      }
      case 'usage': {
        const usage = numberMap(item, ['input_tokens', 'output_tokens'], count);
        if (usage) picked[key] = usage;
        break;
      }
    }
    if (valid) picked[key] = item;
  }
 ...
 */
export function createGate({ cwd, config, pendingPath, onPending, complete, fastScreen, evidenceProbe, getIntent, getPriorActions, appendEvent }) {
  ...
  async function gatePermission(params) {
    ...
    const secret = config?.review?.jev?.apiKey?.trim();
    const reviewMetadata = pickMetadata(result, REVIEW_METADATA_KEYS, secret);
    const fastReview = pickMetadata(result.fastReview, FAST_REVIEW_KEYS, secret);
    const preScreen = pickMetadata(result.preScreen, PRE_SCREEN_KEYS, secret);
    const audit = {
      ...reviewMetadata,
      ...(fastReview === undefined ? {} : { fastReview }),
      ...(preScreen === undefined ? {} : { preScreen }),
    };
    if (result.decision === 'allow') {
      appendEvent({
        type: 'executor.gate', stage: result.stage, decision: 'allow', reason: result.reason,
        ...audit,
      });
