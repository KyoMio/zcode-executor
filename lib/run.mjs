    ...
    for (;;) {
      ...
      for (;;) {
        ...
        const inTurnWatcher = setInterval(() => {
          ...
          // 2026-09-21 对照 ZCode 源码：3.12.2 起回合中 session/send 直接被拒（-32010），插话必失败。
          // 失败要可见：executor.steer_failed 事件 + state.steerUnavailable，send --steer 据此提前拒绝；
          // 真插上了（resolve）就清掉标记。async 包一层是让 handler 里抛错也有地方接（RULES §3）：
          // 之前 then 挂双 handler 的写法，两个 handler 自己抛错都没人接，会成 unhandled rejection
          void (async () => {
            try {
              await session.steer(steer.item.text ?? '');
              writeState({ steerUnavailable: null });
            } catch (err) {
              process.stderr.write(`runner: steer 失败：${err?.message ?? err}\n`);
              appendEvent(eventsPath, { type: 'executor.steer_failed', at: now(), reason: err?.message ?? String(err) });
