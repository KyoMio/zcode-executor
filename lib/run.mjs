...
import { loadZcodeConfig, resolveReviewSelection } from './models.mjs';
import { buildModelSelection, EXECUTOR_PROVIDER_ID, writePersonalProviderFile } from './providers.mjs';
import { reasoningLevelFor } from './tiers.mjs';
import { createGate } from './gate.mjs';
import { createComplete } from './review/complete.mjs';
import { createJevFastScreen } from './review/jev.mjs';
...
/** 只在 runner 内选择快筛；Jev key 不向 app-server 或 provider 文件传播。 */
export function createConfiguredFastScreen(config, options = {}) {
  if (config?.review?.enabled === false) return undefined;
  const apiKey = config?.review?.jev?.apiKey;
  if (apiKey === undefined) return undefined;
  return createJevFastScreen({
    apiKey,
 ...
 * state.json → 写个人 provider 文件 → spawn → attach（3.12 起不推表，D14）→
...
export async function runSession({ home, config, sessionId }) {
  ...
  let sessionRef = null;
  let gate = null;
  let providerFile = null;   // 当前连接的个人 provider 文件（含 apiKey，D14）；连接收场就删，抛错、信号也删
  const onSignal = (signal) => {
    if (signaled) return;
    signaled = true; // writeState 由此定稿：后续流程（投递/挂起轮询）不再改写 state
    try {
      // 2026-09-21 对照 ZCode 源码：session/stop 是带 id 的请求（不再是无 id 的通知），
      // 发不出去记一行 stderr，不影响信号收场时序
      sessionRef?.stop().catch((err) => {
        process.stderr.write(`runner: 收场时的 session/stop 没发出去：${err?.message ?? err}\n`);
      });
    } catch {
      // 连接已经没了
    }
    ...
    // 给 stdin 一点冲刷时间。个人文件（含 apiKey）在真正退出前删（D14）：finally 在 process.exit 面前
    // 不会跑；也不能在这里立刻删——启动阶段收到信号时主流程还在 spawn/create，文件一没 create 立刻
    // 失败、runner 以 1 退出抢在 exit(0) 前面（ops.test 的 SIGTERM 用例复现过）
    setTimeout(() => {
  ...
  const fireCancelStop = () => {
    ...
    try {
      // 2026-09-21 对照 ZCode 源码：session/stop 是带 id 的请求（不再是无 id 的通知），
      // 发不出去记一行 stderr，cancel 照常走宽限断开
      sessionRef?.stop().catch((err) => {
        process.stderr.write(`runner: cancel 的 session/stop 没发出去：${err?.message ?? err}\n`);
      });
    } catch {
      // 连接已经没了
    }
  ...
  try {
    ...
    for (;;) {
      ...
      //    → 找登记簿的 provider → 写个人 provider 文件 → spawn（secrets、个人文件、按 providerId 给 key）
      //    → 连接会话（D13：登记簿没有 sessionId 先 create 并写回；有 sessionId 走 resume，
      //    Session not found 则重建）。3.12 起 app-server 自己从个人文件读表，不再推表（D14）
      const { registry } = loadZcodeConfig();
      const providerObj = registry.providers.find((p) => p.providerId === entry.provider);
      if (!providerObj) {
        // provider 表变了（T6-C 起有两个来源，不指具体文件——账号型登出、config.json 被 App 重写都会走到这）：
        // 会话建不起来也跑不了回合，说清是哪个 provider 没了
        throw new ExecutorError(
          `登记簿里的 provider ${entry.provider} 已不在 provider 表里（账号型登出、或 config.json 变了）。`
            + '重跑 zcode-executor models 看现在有哪些，再 new 一条会话',
          2,
        );
      }
      ...
      providerFile = writePersonalProviderFile(providerObj);
      const client = await AppServerClient.spawn({
        cwd: entry.cwd,
        secrets,
        personalProviderFile: providerFile.path,
        // 个人文件里只有我们这一条 provider，requestProviderRuntimeHeaders 只按这个 id 给 key
        providerAuth: (providerId) => (providerId === EXECUTOR_PROVIDER_ID ? providerObj.apiKey?.value : undefined),
      });
      clientRef = client;
      // T3.2 闸门接线。模型审批的 Complete 经 workspace/generateText：fast 档模型 +
      // review.thought 档（默认 low）；review.enabled:false 或算不出模型时 complete 不传——
      // 闸门除红线外一律转人工（故障时默认安全）。
      let complete;
      let fastScreen;
      if (config.review?.enabled !== false) {
        try {
          const { selection } = resolveReviewSelection({ registry, providerId: entry.provider, config });
          complete = createComplete({ client, workspace, selection, timeoutMs: config.review?.timeoutMs });
          fastScreen = createConfiguredFastScreen(config, { knownSecrets: secrets });
        } catch (err) {
      ...
      if (!zcodeSessionId) {
        // 3.12.2 起 GLM 5.3 系列 create 缺 reasoningLevel 直接被拒，登记簿的 thoughtLevel 又可能是 null，
        // 档位按 reasoningLevelFor 回落；model.options 与顶层 thoughtLevel 两处都给——真机实测只给
        // options 时 settings.thoughtLevel.current 是空的（docs/reference/zcode-app-server-protocol.md「3.12.2 变化」，2026-09-18）
        const reasoningLevel = reasoningLevelFor(entry.thoughtLevel, providerObj.models.find((m) => m.modelId === entry.modelId));
        const createParams = {
          workspace,
          mode: 'build',
          persistence: 'immediate',
          titleGenerationEnabled: false,
          model: buildModelSelection(providerObj, entry.modelId, reasoningLevel),
      ...
      for (;;) {
        ...
        // steerUnavailable 一起清掉（评审 fix2 第 1 条）：「插话不可用」只描述当前回合，新回合开始就重置
        writeState({ phase: 'running', current: { text, task, startedAt: now() }, steerUnavailable: null });
        ...
        const inTurnWatcher = setInterval(() => {
          ...
          // 2026-09-21 对照 ZCode 源码 / 本机探针：3.12.2 起插话走 v4/command 的 sendText
          // （requestedDelivery:"guide"，下一个工具边界生效；没有边界时排到回合结束后执行）。
          // 插话正文先留痕（不管成败）：取到就记 executor.steer（带 text），ACK 回来另记
          // executor.steer_accepted（带 delivery 与 status）；失败要可见：executor.steer_failed
          // 事件（带 text）+ state.steerUnavailable，send --steer 据此提前拒绝；真插上了就清掉标记。
          // async 包一层是让 handler 里抛错也有地方接（RULES §3）：
          // 之前 then 挂双 handler 的写法，两个 handler 自己抛错都没人接，会成 unhandled rejection
          appendEvent(eventsPath, { type: 'executor.steer', at: now(), text: steer.item.text ?? '' });
          void (async () => {
            try {
              const ack = await session.steer(steer.item.text ?? '');
              appendEvent(eventsPath, { type: 'executor.steer_accepted', at: now(), delivery: ack.delivery, status: ack.status });
              writeState({ steerUnavailable: null });
            } catch (err) {
              process.stderr.write(`runner: steer 失败：${err?.message ?? err}\n`);
              appendEvent(eventsPath, { type: 'executor.steer_failed', at: now(), text: steer.item.text ?? '', reason: err?.message ?? String(err) });
              writeState({ steerUnavailable: err?.message ?? String(err) });
            }
          })().catch((e) => {
            // catch 里的落盘本身也可能抛（目录被清、磁盘满）：兜住，不让 runner 因记账失败而崩
            process.stderr.write(`runner: steer 失败记录没写成：${e?.message ?? e}\n`);
          });
        }, STEER_POLL_MS);
        ...
        // 底层可能是 stop 生效后的 resultType=cancelled（session 结算已是 cancelled）、
        // 也可能是宽限断开后的 exited，对外只说「已被 cancel 叫停」，退出码表里 cancelled: 4 由此激活
      ...
      sessionRef = null;
      gate = null;
      providerFile.dispose(); // 连接已收场，个人文件（含 apiKey）没人再读（D14）；回圈重连会重写一份
    ...
    providerFile?.dispose(); // 抛错、cancel 路径也删（D14；信号路径在 onSignal 的退出定时器里删）；dispose 可重复调
